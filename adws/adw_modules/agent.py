...
class AgentPromptRequest(BaseModel):
    ...
    adw_id: str
    agent_name: str = "ops"
    model: Literal["sonnet", "opus", "fable"] = "sonnet"
...
# Environment: the kit's own .env first, then the current repo's .env (repo values win)
...
def agents_root(working_dir: Optional[str] = None) -> str:
    """Directory that holds run outputs: <working_dir>/agents (F1: never the kit root)."""
    return os.path.join(working_dir or os.getcwd(), "agents")
...
def save_prompt(prompt: str, adw_id: str, agent_name: str = "ops", working_dir: Optional[str] = None) -> None:
    ...
    # Store next to the run outputs, inside the repo being operated on
    prompt_dir = os.path.join(agents_root(working_dir), adw_id, agent_name, "prompts")
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    save_prompt(request.prompt, request.adw_id, request.agent_name, request.working_dir)
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    # Output directory lives in the repo being operated on: <working_dir>/agents/<adw_id>/<agent>
    output_dir = os.path.join(agents_root(request.working_dir), request.adw_id, request.agent_name)
    os.makedirs(output_dir, exist_ok=True)
